...
/* $Id: phg.h,v 1.187 2022/06/09 07:28:31 zlb Exp $ */
...
#define BDRY_SHIFT 3
#define BDRY_MASK  ((BTYPE)((((unsigned)(-1)) >> BDRY_SHIFT) << BDRY_SHIFT))
...
#endif
    ELEM_TYPE           elem_type;              /**< Elem type (elem-info.h) */
} ELEMENT;
...
typedef struct {
    ...
    BYTE	rface[NVertFace];
				/* the VERTEX indices of the shared face in the
				 * remote element, i.e.:
				 *	rface[i] (0-3) is the index of the
				 *	remote VERTEX matching the local VERTEX
				 *	GetFaceVertex(.,i), i=0,1,2 */
    BYTE	vertex;		/* index of the the shared face (0-3) */
    BYTE	op_vertex;	/* index of the the shared face in the
				 * remote element */
    BYTE	depth;		/* refinement count (depth) of the element */
} RNEIGHBOUR;
#endif

typedef struct HALO_	HALO;
...
struct GRID_ {
    ...
    ELEMENT	**elems;	/**< Array of size g->nlem, pointers to leaf
					 ...
					 assert(elems[i] == NULL ||
						elems[i]->index == i) */
    ELEM_TYPE	elem_type;	/**< Grid element types */  
    ...
    /* inter-grid links */
    GRID	*alien;	/**< Pointer to another grid */
    ...
    INT		i;		/**< loop index used by ForAllElements */

    /* Note: "serial_no" is incremented by 1 each time the mesh is refined or
     * coarsened and by 65536 if the mesh is repartitioned. */
    ...
    /* face-to-element map: face i =>
     * 		'Fmap_faces[i]'-th face of the element 'Fmap_elems[i]' */
    ELEMENT	**Fmap_elems;
    BYTE	*Fmap_faces;
};
...
#include "phg/nonlinear-solver.h"
