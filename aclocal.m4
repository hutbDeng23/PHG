# $Id: aclocal.m4,v 1.27 2020/10/20 12:36:55 zlb Exp $
...
AC_DEFUN(AC_FULLPATH, [dnl
    dnl Split the command ($1) into cmd and args
    dnl WARNING: it's assumed that $1 does not contain '!'
...
AC_DEFUN(AC_FIND_HEADER, [dnl
	...
	for ac_d in $2 /usr/include/$3 /usr/$3/include dnl
		 ...
		 /usr/local/$3/include ${MPI_INCLUDE}/$3
	do
...
#
# AC_FIND_LIB(name, dirs, dirname_patterns, libname_list, testprog)
#
AC_DEFUN(AC_FIND_LIB, [dnl
    if test x"${with_$1_lib}" = x; then
	ac_flag=false
	for ac_d in $2 /usr/lib* /usr/lib*/$3 /usr/$3/lib* dnl
		    /opt/lib* /opt/local/lib* /opt/lib*/$3 /opt/$3/lib* dnl
		    /usr/local/lib* /usr/local/lib*/$3 /usr/local/$3/lib*
	do
	    for ac_n in $4; do
		for ac_f in ${ac_d}/lib${ac_n}.{so,a,dylib,lib}; do
		    if test -r "${ac_f}"; then
			with_$1_lib="${with_$1_lib} `echo ${ac_f}`"
		    fi
		done
	    done
	done
	...
	if ${ac_flag}; then
	    ...
	    dnl strip leading 'lib' and trailing extension
	    ac_base=`echo $ac_base | sed \
			-e 's/^lib//' dnl
			-e 's/\.a$//' dnl
			-e 's/\.so$//' dnl
			-e 's/\.dylib$//' dnl
	...
	if test ${enable_$1} = no; then
	    for ac_tmp in $4; do
		ac_lib="-l${ac_tmp}"
		LIBS="${ac_lib} ${LIBS_findbak}"
		AC_LINK_IFELSE([$5], enable_$1=yes, enable_$1=no)
		if test "enable_$1" != "no"; then break; fi
	    done
	fi
	if test ${enable_$1} != no; then
	    with_$1_lib=${ac_lib}
...
AC_DEFUN(AC_CHECK_LIBHEADER, [dnl
    AC_MSG_CHECKING([whether we have lib$1 ])
    LIBS_bak="$LIBS"
    AC_ADD_LIBS(LIBS, "-l$1")
    AC_LINK_IFELSE([$3
		main() {$2;}], have_lib$1=yes, have_lib$1=no)
    AC_MSG_RESULT(${have_lib$1})
    if test "${have_lib$1}" = "yes"; then
	AC_DEFINE(m4_toupper([HAVE_LIB$1]), 1)
    else
	LIBS="$LIBS_bak"
	AC_DEFINE(m4_toupper([HAVE_LIB$1]), 0)
    fi
])

dnl AC_DEFUN(AC_CHECK_SIZEOF0, [dnl
dnl     dnl replace ' ' with '_': 
dnl     define(AC_TMP, translit(ac_cv_sizeof_$1, [ *], [_p]))dnl
dnl     AC_MSG_CHECKING([size of $1 ])
dnl     AC_CACHE_VAL(AC_TMP, [
dnl 	AC_TRY_RUN([
dnl #include <stdio.h>
dnl main()
dnl {
...
dnl   fprintf(f, "%d\n", sizeof($1));
dnl   exit(0);
dnl }], AC_TMP=`cat conftestval`, AC_TMP=0, ifelse([$2], , , AC_TMP=$2))])dnl
dnl     AC_MSG_RESULT($AC_TMP)
dnl ])
