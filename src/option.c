...
/* $Id: option.c,v 1.191 2022/09/22 07:08:33 zlb Exp $ */
...
/* callback function */
static BOOLEAN callback_flag = FALSE;
void (*_phg_modify_options_callback)(void) = NULL;

static OPTION *options = NULL;
static int *options_index = NULL;
static size_t options_count = 0, options_allocated = 0;

static BOOLEAN warn_nonopt = TRUE;
...
static void set_option0(OPTION *o, const void *value,
				const char *arg, const char *prefix);
static void set_option(const char *op_name, void *value, int type,
			  const char *func);
...
OBSOLETE_OPTION obsolete_options[] = {
    ...
    {"mumps_symmetry",			"solver_symmetry",	FALSE},
    {"hips_symmetry",			"solver_symmetry",	FALSE},
    {"pastix_symmetry",			"solver_symmetry",	FALSE},
    {"spooles_symmetry",		"solver_symmetry",	FALSE},
    {"pardiso_symmetry",		"solver_symmetry",	FALSE}
};
...
option_sort(void)
{
    ...
    phgOptionsRegister("dummy", NULL, NULL, NULL, VT_NONE, FALSE);
...
static void
option_register(OPTION *o, const char *name, const char *help, const char **keys,
...
void
phgOptionsRegister(const char *name, const char *help, const char **keys,
		   void *var, VTYPE type, BOOLEAN append)
{
    OPTION *o;
    static BOOLEAN initialized = FALSE;
	...
	phgOptionsRegister("-options_file", "Read options from given file",
	...
	phgOptionsRegisterNoArg("-warn_nonopt",
			"Warn on non-option arguments",
			&warn_nonopt);
	...
	phgMatConditionNumber(NULL);	/* register -cond_* options */
#if USE_MPI
	phgInitMPI(NULL, NULL);		/* register MPI options */
#endif	/* USE_MPI */
	phgIOOpen(MPI_COMM_SELF, NULL);		/* Register I/O options */
    ...
    option_register(o, name, help, keys, var, type, append);
}

void
phgOptionsModify(const char *name, const char *help, const char **keys,
		 void *var, VTYPE type, BOOLEAN append)
/* This function modifies (re-sets) an existing option */
{
    int *k;
    OPTION *o;

    if (!callback_flag)
	phgError(1, "%s can only be called in _phg_modify_options_callback!\n",
		    __func__);

    assert(name != NULL && type != VT_INIT && type != VT_TITLE);
...
phgOptionsPop(void)
{
	...
	if (stack[ps][i] != NULL) {
	    ...
	    o->append = FALSE;
	    set_option0(o, stack[ps][i], NULL, NULL);
   ...
   the argument list */
{
	...
	if (options == NULL) {
	    ...
	    phgOptionsRegister(NULL, NULL, NULL, NULL, VT_NONE, FALSE);
	}
	option_sort();
	...
	for (i = 1; i < options_count; i++) {
		...
		phgRank == 0)
		phgWarning("multiply defined option \"-%s\".\n",o->name);
	    j = i;
	}

	/* the callback function for modifying some options */
	if (_phg_modify_options_callback != NULL) {
	    callback_flag = TRUE;
	    _phg_modify_options_callback();
	    callback_flag = FALSE;
	}
	...
	if ((p = (*argv)[i])[0] == '-' || p[0] == '+') {
	    ...
	    if (warn_nonopt && phgRank == 0)
		phgWarning("appending \"%s\" to -oem_options (ungrouped multi-word arguments?).\n", (*argv)[i]);
	...
	if (o->type != VT_NONE) {
	    set_option0(o, NULL, arg, prefix);
	}
	else {
	    BOOLEAN flag = p[0] == '-' ? TRUE : FALSE;
	    assert(*prefix == '\0');
	    set_option0(o, (const void *)&flag, NULL, NULL);
	}
    }
...
static void
set_option0(OPTION *o, const void *value, const char *arg, const char *prefix)
{
    char *p, **pp, *endptr;
    size_t size;
    FLOAT fval;
    ...
    assert((value == NULL) != (arg == NULL));
    assert(arg == NULL || prefix != NULL);
	...
	case VT_INT:
		...
		*(INT *)o->var = *(INT *)value;
	    ...
	    ival = (INT)strtol(arg, &endptr, 10);
	    if (*endptr != '\0')
		phgError(1, "invalid argument \"%s\" for option -%s\n",
			    arg, o->name);
	    switch (*prefix) {
		case '\0':  *(INT *)o->var  = ival; break;
		case '+':   *(INT *)o->var += ival; break;
		case '-':   *(INT *)o->var -= ival; break;
		case '*':   *(INT *)o->var *= ival; break;
		case '/':   *(INT *)o->var /= ival; break;
		case '%':   *(INT *)o->var %= ival; break;
	...
	case VT_FLOAT:
		...
		*(FLOAT *)o->var = *(FLOAT *)value;
	    ...
	    fval = (FLOAT)strtold(arg, &endptr);
	    if (*endptr != '\0')
		phgError(1, "invalid argument \"%s\" for option -%s\n",
			    arg, o->name);
	    switch (*prefix) {
		case '\0':  *(FLOAT *)o->var  = fval; break;
		case '+':   *(FLOAT *)o->var += fval; break;
		case '-':   *(FLOAT *)o->var -= fval; break;
		case '*':   *(FLOAT *)o->var *= fval; break;
		case '/':   *(FLOAT *)o->var /= fval; break;
		default:    phgError(1, "Invalid option \"-%s%c=\".\n",
	...
	case VT_FILENAME:
	    ...
	    if (value != NULL) {
		phgFree(*(char **)o->var);
		*(char **)o->var = value == NULL_STRING ?
					NULL : strdup((const char *)value);
		break;
	    }
	    if (*prefix == '+' && o->type == VT_FILENAME)
		phgError(1, "Invalid option \"-%s%c=%s\".\n",
						    o->name, *prefix, arg);
	    /* Note: any prefix other than '+' cancels "append" */
	    if (*prefix == '+' || (*prefix == '\0' && o->append)) {
		size_t size1;
		/* append 'arg' to existing string */
		size1 = strlen(arg) + 1;
		/* skipping trailing spaces in p */
		while (size > 1 && isspace(p[size - 2]))
		    size--;
		if (size == 1)
		    size = 0;	    /* empty string */
		*(char **)o->var = p =
	...
	case VT_KEYWORD:
	    ...
	    o->used = TRUE;
	    if (value != NULL)
		arg = value == NULL_STRING ? NULL : value;
	...
	case VT_HANDLER:
	    ...
	    /* Note: any prefix other than '+' cancels "append" */
	    else if (*prefix == '+' || (*prefix == '\0' && o->append)) {
		j = strlen(o->keys[0]);
		o->keys[0][j++] = ' ';
	    }
	    else {
...
{
    set_option(op_name, &value, VT_NONE, __func__);
    return TRUE;
}
...
{
    set_option(op_name, &value, VT_INT, __func__);
    return TRUE;
}
...
{
    set_option(op_name, &value, VT_FLOAT, __func__);
    return TRUE;
}
...
{
    set_option(op_name, (void *)value, VT_KEYWORD, __func__);
    return TRUE;
}
...
{
    set_option(op_name, (void *)value, VT_STRING, __func__);
    return TRUE;
}
...
{
    set_option(op_name, (void *)value, VT_FILENAME, __func__);
    return TRUE;
}
...
{
    set_option(op_name, (void *)value, VT_HANDLER, __func__);
    return TRUE;
}
