...
/* $Id: utils.c,v 1.478 2022/08/31 06:11:03 zlb Exp $ */

#define _GNU_SOURCE	/* for fcloseall(), feenableexcept(), etc. */
...
static char exe_fn[PATH_MAX] = "undefined";
const char *phgExeFn = exe_fn;
int phgMaxThreads = 1, phgThreadId = 0;
int phgMaxThreads_dummy = 1, phgThreadId_dummy = 0;	/* see DONT_WANT_OMP */
INT _phg_omp_i = -1;	/* used by ForAllElements{Begin,End} */
INT phgVerbosity = 0;
const char *phgCurrentFunctionName_ = "PHG";

/* some global variables which can be used in debugging */
INT	_phg_debug_i = 0;
FLOAT	_phg_debug_f = 0.;
void *	_phg_debug_p = NULL;

static BTYPE default_bdry_type = UNDEFINED;

/* function used by ForAllElements macro */
ELEMENT *
_phg_step_element(GRID *g, INT *i, FORALL_TYPE for_what)
/* Note: 'for_what' controls to loop over which subset of the elements */
{
    INT n0 = 0, n = 0;
    ELEMENT *e = NULL;

    if (for_what == FOR_NONE || (g->halo == NULL && for_what == FOR_UNOWNED)) {
	*i = -1;
	return NULL;
    }
	...
	case FOR_ALL:
	    ...
	    break;
	default:
	    phgError(1, "%s: invalid 'for_what' value: %d!\n", for_what);
    }

    while (*i < n && (e = g->elems[*i + n0]) == NULL)
	(*i)++;
...
INT
_phg_step_face(GRID *g, INT i, ELEMENT **e, BYTE *face)
{
    INT n = g->halo == NULL ? g->nface : g->halo->nface_bak;

    while (i < n && !(g->types_face[i] & OWNER))
	i++;

    if (i >= n)
	return -1;

    assert(g->Fmap_elems != NULL);
...
#if HAVE_FEENABLEEXCEPT
void (*__phg_user_fpe_handler)(int, siginfo_t *, void *) = NULL;

static void
fpe_handler(int signum, siginfo_t *siginfo, void *dummy)
{
    if (__phg_user_fpe_handler != NULL) {
	__phg_user_fpe_handler(signum, siginfo, dummy);
	return;
    }
 ...
 * of all external packages (like PETSc) */
{
    ...
    if (init) {
	/* This one is temporarily here and is to be moved else where */
	phgOptionsRegisterNoArg("-back_trace",
		"Make phgError() to print a back trace.", &bt_flag);
...
print_processes(void)
{
    ...
    if (phgRank == 0) {
	fprintf(stderr, "List of MPI processes:\n");
...
#if 0
    exe_fn[readlink("/proc/self/exe", exe_fn, 511)] = 0;
#elif defined(__WIN__)
    _fullpath(exe_fn, (*argv)[0], PATH_MAX - 1);
#else
    if (realpath((*argv)[0], exe_fn) == NULL) {
	/* make gcc happy with the if (...) above */
    }
#endif
...
#pragma omp parallel
    ...
    phgNonlinearSolver(0, NULL, NULL, NULL, 0, 0., 0., NULL, NULL);
    phgQuadInterface(NULL, NULL, NULL, 0, NULL, NULL, NULL);
    phgXFEMInit(NULL, NULL, 0, 0);
...
#if USE_MPI
    if (!phgMasterSlave)
	/* tell idle processes to exit */
	phgCreateComm_(-1, phgComm, NULL);
#endif	/* USE_MPI */
...
#if USE_MPI
typedef struct {
    INT		index;	/* global index of the vertex or edge */ 
    INT		lindex;	/* local vertex or edge index */ 
...
/* Note: when two elements in different processes are connected only by a
 * common vertex or edge (and by no other elements), with the present code the
 * vertex or edge will be owned by both processes, which leads to inconsistent
 * numbers between {nvert|edge}_owned and {nvert|edge}_global, and incorrect
 * edge numbering (the common edge has different global index in 2 processes),
 * and the function build_maps() would fail because of the inconsistencies.
 *
 * Some examples showing this problem:
 *	mpirun -np 2 ./poisson -mesh_file ../test/vertex-neighbours.mesh
 *	mpirun -np 2 ./poisson -mesh_file ../test/edge-neighbours.mesh
 *	mpirun -np 4 ./poisson -mesh_file ../TrueGrid-files/piston.mesh.bz2
 *
 * Note: piston.mesh.bz2 is a very bad mesh with various problems: duplicate
 *	 vertices, bad and non-conforming elements (e.g., the elements
 *	 containing the vertex 3572 (3573 in the input file))
 *
 * The option "-allow_degenerate_neighbours" is added as a workaround for this
 * problem. At present it only ensures the following conditions and prevents
 * build_maps() from raising an error, at the cost of notably increased
 * complexities, in both communications and computations, in the function
 * phgUpdateBoundaryTypes:
 *   1. The global numbering and the counters nvert_owned and nvert_global are
 *	consistent across processes.
 *   2. The global numbering of edges are not consistent (a same edge may get
 *	different global indices in different processes), but the counters
 *	nedge_owned and nedge_global are consistent, and they conform to the
 *	global edge numbering.
 *
 * TODO: fix e->bound_type[], UpdateEdges, etc., to have consistent global
 *	 edge numbering
 */
static BOOLEAN allow_degenerate_neighbours = FALSE;
...
default_bdry_callback CB_ARGS(e)
{
	...
	if (e->bound_type[i] == UNDEFINED) {
	    ...
	    e->bound_type[i] |= default_bdry_type;
	}
    }
 ...
 * and the peer_face member of g->neighbours. */
{
    ...
    /* map UNDEFINED boundary type to default type */
    /* Note: can't use ForAllElements because g->elems is not up to date */
    if (default_bdry_type != UNDEFINED)
	...
	g->owner_index_vert =
		phgAlloc(g->nvert * sizeof(*g->owner_index_vert));
	g->nvert_owned = 0;
	nd = n0 = -1;
	for (i = 0; i < g->nvert; i++) {
     ...
     *	  Each entity (vertex or edge) with the REMOTE flag set is assigned to
     *	  a process.
     *
     *	  The vertices are assigned to processes using global vertex indices
     *	  as follows:
     *		The nvert_global vertices are first divided into nprocs blocks,
     *		the size of the first nvert_global%nprocs blocks is
     *			nvert_global / nprocs + 1,
     *		and the size of the remaining blocks is
     *			nvert_global / nprocs,
     *		then vertices in each block are assigned to the corresponding
     *		process.
     *
     *	  The edges are assigned similarly using global edge indices.
     *
     *	  All entities with the REMOTE flag set are sent to their assigned
     *	  processes, processed there, and then sent back with updated info. */

#define IsRemote(t)    ((b = (t)) & REMOTE)

    /* count entities (vertices or edges with the REMOTE bit set) to be sent,
     * number of entities for process 'rank' is stored in scnts[rank] */
	...
	n0 = nm * (nd + 1);
	for (i = 0; i < g->nvert; i++) {
	    if (IsRemote(g->types_vert[i]) || allow_degenerate_neighbours) {
		j = GlobalVertexP(g, i);
    ...
    /* sort the list with respect to type (vertex or edge) and global index */
	 ...
	 * it will be set to be owned by the process with smallest value of
	 *		(index + rank) % nprocs,
	 * where 'index' is the global index of the entity and 'rank' is
	 * the rank of the incoming process.
	 */
	rank = -1;	/* to be set to the owner process rank */
	n0 = -1;	/* to be set to the local index in the owner process */
	b = 0;		/* to be set to BTYPE */
	i = g->nprocs;
	for (jj = ii; jj < n; jj++) {
	    bl = brecv + rind[jj];
	    b |= bl->btype;
	    ...
	    assert(bl->btype != UNREFERENCED || allow_degenerate_neighbours);
	    if (i > j) {
		i = j;
		rank = bl->rank;
		n0 = bl->lindex;
	...
	if (allow_degenerate_neighbours && bl->gtype == VERTEX) {
	    if (n - ii <= 1) {
		b |= OWNER;
		b &= ~REMOTE;
	    }
...
print_trace(void)
{
    ...
    /* First try gdb, see:
     * https://stackoverflow.com/questions/3151779/how-its-better-to-invoke-gdb-from-program-to-print-its-stacktrace?noredirect=1 */
    char pid_buf[30];
    int child_pid;
    pid_t pid = getpid();

    sprintf(pid_buf, "--pid=%d", (int)pid);

    phgInfo(-1, "Stack trace for %s pid=%d\n", phgExeFn, (int)pid);
    fflush(LogFile);
	...
	execlp("gdb", "gdb", "--batch", "-n", "-ex", "thread", "-ex", "bt",
		phgExeFn, pid_buf, NULL);
	/* If gdb failed to start */
#if !HAVE_EXECINFO_H
	abort();
#else
	/* call backtrace() */
	static void *bt_buffer[1024];
	static int bt_size = sizeof(bt_buffer) / sizeof(bt_buffer[0]);
	int size;
	size = backtrace(bt_buffer, bt_size);
	backtrace_symbols_fd(bt_buffer, size, fd);
#endif
    } else {
	waitpid(child_pid, NULL, 0);
    }

    phgInfo(-1, "End stack trace\n");
    fflush(LogFile);
#endif	/* !defined(__WIN__) */
}
...
#undef VERTEX
    ...
    phgInfo(-1, "unexpected error in %s: face=%d\n", __func__, v);
    phgInfo(-1, "    e: L=(%d,%d,%d,%d), G=(%d,%d,%d,%d), P=(%d,%d,%d,%d)\n",
	    e->verts[0], e->verts[1], e->verts[2], e->verts[3],
	    GlobalVertex(g, e->verts[0]), GlobalVertex(g, e->verts[1]),
	    GlobalVertex(g, e->verts[2]), GlobalVertex(g, e->verts[3]),
	    GlobalVertexP(g, e->verts[0]), GlobalVertexP(g, e->verts[1]),
	    GlobalVertexP(g, e->verts[2]), GlobalVertexP(g, e->verts[3]));
    phgInfo(-1, " e_op: L=(%d,%d,%d,%d), G=(%d,%d,%d,%d), P=(%d,%d,%d,%d)\n",
	    e_op->verts[0], e_op->verts[1], e_op->verts[2], e_op->verts[3],
	    GlobalVertex(g, e_op->verts[0]), GlobalVertex(g, e_op->verts[1]),
	    GlobalVertex(g, e_op->verts[2]), GlobalVertex(g, e_op->verts[3]),
	    GlobalVertexP(g, e_op->verts[0]), GlobalVertexP(g, e_op->verts[1]),
	    GlobalVertexP(g, e_op->verts[2]), GlobalVertexP(g, e_op->verts[3]));
    phgError(-1, "abort.\n");
...
phgFreeElement(ELEMENT **eptr)
{
	...
	return;
#if 0
    /* note: should add a flag owns_user_ctx if need to free user_ctx here */
    if (e->user_ctx != NULL)
	phgFree(e->user_ctx);
#endif
    phgFree(e);
...
phgFreeGrid_(GRID *g)
{
	...
	if (s->children[1] != NULL)
	    free_grid_traverse(s->children[1]);
#if 0
	/* should add a flag owns_user_ctx if need to free user_ctx here */
	if (e->user_ctx != NULL)
	    phgFree(e->user_ctx);
#endif
    }
...
#if 0
    /* WARNING: range check skipped, since it would fail with, e.g., the
     * temporary grid created by phgMapInitNeighbourMap(), which has bogus
     * L2Gmap_{vert,elem} */
    for (i = 0; i < n; i++) {
	INT j = map[i];
	if (j < 0 || j >= nglobal)
...
#if USE_MPI
    CheckL2Gmap(g->L2Gmap_vert,
		g->halo != NULL ? g->halo->nvert_bak : g->nvert,
		g->nvert_global);
    if ((g->flags & EDGE_FLAG))
	CheckL2Gmap(g->L2Gmap_edge,
		    g->halo != NULL ? g->halo->nedge_bak : g->nedge,
		    g->nedge_global);
    if ((g->flags & FACE_FLAG))
	CheckL2Gmap(g->L2Gmap_face,
		    g->halo != NULL ? g->halo->nface_bak : g->nface,
		    g->nface_global);
    if ((g->flags & ELEM_FLAG))
	CheckL2Gmap(g->L2Gmap_elem,
		    g->halo != NULL ? g->halo->nelem_bak : g->nelem,
		    g->nelem_global);
#endif
...
/* TODO: patch composed of several non contiguous parts */
{
    ...
    *p = phgAlloc((size = 6) * sizeof(**p));
	...
	if (e1 == e) {
	    int i;
	    phgInfo(-1, "error: e1==e (v0,v1,v2,v3 = %d,%d,%d,%d, count0=%d)\n",
			v0, v1, v2, v3, count0);
	    for (i = 0; i < count; i++)
...
#if Dim == 2
    phgInfo(-1, "E(%"dFMT"): %p, %s, marks=%d:%d, bdry=%x:%x:%x, " "gen=%d\n",
		(g->flags & ELEM_FLAG) && e->index >= 0 ?
				GlobalElement(g, e->index) : (INT)(-1),
		e, GTypeName(e->type), e->mark, e->region_mark,
...
#endif
#else	/* Dim */
    phgInfo(-1, "E(%"dFMT"): %p, %s, marks=%d:%d, bdry=%x:%x:%x:%x, gen=%d\n",
		(g->flags & ELEM_FLAG) && e->index >= 0 ?
...
{
    if (bcmap[*(const int *)p0].min < bcmap[*(const int *)p1].min)
	return -1;
    if (bcmap[*(const int *)p0].min > bcmap[*(const int *)p1].min)
	return 1;
    if (bcmap[*(const int *)p0].max < bcmap[*(const int *)p1].max)
	return -1;
    if (bcmap[*(const int *)p0].max > bcmap[*(const int *)p1].max)
	return 1;
...
const char *
phgBTypesString(BTYPE btype)
/* returns a string for the bdry masks in "btype" */
{
    static char str[150], *p, *q;
#if USE_OMP
# pragma omp threadprivate(str)
    ...
    *(p = str) = '\0';
    for (mask = (1 << BDRY_SHIFT); mask <= BDRY_MASK; mask <<= 1) {
	if (!(mask & btype))
	    continue;
	if (str[0] != '\0')
	    *(p++) = '|';
...
int
phgBTypesValue(const char *btype_string)
/* converts a string like "Dirichlet|bdry_user0" to BTYPE value and returns it,
 * returns UNDEFINED if btype_string == NULL, or -1 if error. */
{
    int i, btype;
    char *q0, *q, *r, *t;
	...
	if ((r = strchr(q, '|')) == NULL)
	    r = strchr(q, '+');
	else if ((t = strchr(q, '+')) != NULL && r > t)
	    r = t;
	if (r == NULL)
	    r = q + strlen(q);
	else
	    *(r++) = '\0';
	for (i = (1 << BDRY_SHIFT); i <= BDRY_MASK; i <<= 1)
	    if (!strcasecmp(BTypeName(i), q))
		break;
	if (i > BDRY_MASK) {
	    free(q0);
	    return -1;
    ...
    free(q0);
    return btype == 0 ? -1 : btype;
}

int
phgLoadBCmap(const char *mesh_fn)
/* A bcmap file defines how to map boundary types in a input mesh file to PHG's
 * boundary masks. Each line in the bcmap_file is composed of two fields
 * separated by spaces in the following form:
 * 	val[:val1] type[|type1[...]]
 * where:
 *   1.	The first column specifies an integer interval, where "val" and "val1"
 *	can be either an integer, or a "*", or omitted (for -infty or +infty).
 *	A single '*' or ':' in the first column means -infty:+infty.
 *   2. "type" can one of "DIRICHLET", "NEUMANN", "BDRY_USER0"..."BDRY_USER9",
 *	and "UNDEFINED" (case insensitive), see include/phg.h for their values.
 *   3.	"|" means bitwise OR, and the "BDRY_" prefix in "type" can be omitted.
 *   4.	optional spaces before the 1st and after the 2nd field are allowed.
 *   5. characters following "#" are treated as comments and ignored.
 * When loading an Medit or Gambit mesh file, the input type is checked against
 * the list of intervals and assigned the boundary type of the first interval
 * it belongs to. Input types not belonging to any intervals are mapped to
 * "UNDEFINED", which in turn is mapped to the boundary type defined by the
 * option "-default_bdry_type".
 *
 * Example:
 * 	-1	neumann		# -1 -> NEUMANN (16)
 * 	0:*	dirichlet	# >=0 -> DIRICHLET (8)
 * 	*	user0|user1	# all others -> BDRY_USER0|BDRY_USER1 (96)
 */
{
    FILE *f;
    char *p, *q, *r, *fn = NULL, line[1024];
    const char *s;
    BOOLEAN star;
    int i, j, k, lineno;
    int *ordering;
	...
	fn = strdup(bcmap_file);
    }
    else if (auto_bcmap && mesh_fn != NULL) {
	/* construct bcmap filename by replacing file extension with .bcmap */
	char *p, *q;
	fn = phgAlloc((k = strlen(mesh_fn)) + 6 + 1);
	strcpy(fn, mesh_fn);
	if ((p = strrchr(fn, '.')) == NULL ||
	    ((q = strrchr(fn, '/')) != NULL && p < q))
	    p = fn + k;
	strcpy(p, ".bcmap");
    ...
    if (phgVerbosity > 0)
	phgPrintf("Reading bcmap file \"%s\":\n", fn);
	...
	if (*p == '\0')
	    continue;
	/* find next token */
	for (r = p; *r != '\0' && !isspace(*r); r++);
	/* skip spaces */
	for (q = r; *q != '\0' && isspace(*(BYTE *)q); q++);
	if (*q == '\0')
	    phgError(1, "%s: syntax error in %s, line %d.\n", __func__, 
			fn, lineno);
	*r = '\0';
	/* remove spaces in the remaining part pointed by q */
	for (s = q, r = q; *s != '\0'; s++)
	    if (!isspace(*s))
		*(r++) = *s;
	*r = '\0';
	/* now p points to the 1st token (range), q points to the 2nd token */
	if (bcmap_n >= bcmap_alloc) {
	    bcmap = phgRealloc_(bcmap, (bcmap_alloc + 16) * sizeof(*bcmap),
				bcmap_alloc * sizeof(*bcmap));
	    bcmap_alloc += 16;
	}
	if ((i = phgBTypesValue(q)) < 0)
	    phgError(1, "%s:%d: syntax error.\n", fn, lineno);
	bcmap[bcmap_n].btype = i;
	/* get min value */
	if (*p == '*' || *p == ':') {
	    star = TRUE;
	...
	if (bcmap[bcmap_n].min > bcmap[bcmap_n].max)
	    phgError(1, "%s:%d: invalid data.\n", fn, lineno);
	if (*r != '\0')
	    phgError(1, "%s:%d: syntax error.\n", fn, lineno);
	k = bcmap_n++;
	/* remove/split overlapping part with intervals 0, ..., k-1 */
	for (i = 0; i < k && k < bcmap_n; i++) {
	    for (j = k; j < bcmap_n; j++) {
		if (bcmap[i].min > bcmap[j].max || bcmap[i].max < bcmap[j].min)
		    continue;	/* non overlapping */
		if (bcmap[i].min <= bcmap[j].min &&
		    bcmap[i].max >= bcmap[j].max) {
		    /* j-th is entirely contained in i-th, delete j-th:
		     *		i	[..........]
		     *		j	   [....]
		     */
		    bcmap_n--;
		    if (j < bcmap_n)
			memmove(bcmap + j, bcmap + j + 1,
				(bcmap_n - j) * sizeof(*bcmap));
		    j--;
		    continue;
		}
#ifdef P1
# undef P1
#endif
#define P1(i)	((i) == INT_MAX ? INT_MAX : (i) + 1)
#ifdef M1
# undef M1
#endif
#define M1(i)	((i) == INT_MIN ? INT_MIN : (i) - 1)
		if (bcmap[i].min > bcmap[j].min &&
		    bcmap[i].max < bcmap[j].max) {
		    /* i-th is strictly contained in j-th, split j-th into 2:
		     *		i	    [....]     ->     [....]
		     *		j	[............] -> [..]      [..]
		     */
		    if (bcmap_n >= bcmap_alloc) {
			bcmap = phgRealloc_(bcmap,
					(bcmap_alloc + 16) * sizeof(*bcmap),
					bcmap_alloc * sizeof(*bcmap));
			bcmap_alloc += 16;
		    }
		    if (j < bcmap_n - 1)
			memmove(bcmap + j + 2, bcmap + j + 1,
					(bcmap_n - j - 1) * sizeof(*bcmap));
		    bcmap[j + 1].min = P1(bcmap[i].max);
		    bcmap[j + 1].max = bcmap[j].max;
		    bcmap[j + 1].btype = bcmap[j].btype;
		    bcmap[j].max = M1(bcmap[i].min);
		    j++;
		    bcmap_n++;
		}
		if (bcmap[i].min <= bcmap[j].min) {
		    /* partially overlapping, shrink j-th
		     *		i	[..........]	  -> [..........]
		     *		j	     [..........] ->             [....]
		     */
		    assert(bcmap[i].max < bcmap[j].max);
		    bcmap[j].min = P1(bcmap[i].max);
		    continue;
		}
		else {
		    /* partially overlapping, shrink j-th
		     ...
		     */
		    assert(bcmap[i].max >= bcmap[j].max &&
			   bcmap[i].min > bcmap[j].min);
		    bcmap[j].max = M1(bcmap[i].min);
		    continue;
		}
		phgError(1, "%s:%d: bcmap[i]=[%d,%d], bcmap[j]=[%d,%d]\n",
			 __FILE__, __LINE__, bcmap[i].min, bcmap[i].max,
			bcmap[j].min, bcmap[j].max);
#undef P1
#undef M1
	    }
	}
    ...
    /* sort and merge/split intervals in bcmap[] */
    ...
    for (i = k = 0; i < bcmap_n; i++, k++) {
	bcmap1[k] = bcmap[ordering[i]];
	if (phgVerbosity > 0) {
	    if (bcmap1[k].min == INT_MIN)
		p = "-infty";
	    else
		sprintf(p = line, "%d", bcmap1[k].min);
	    if (bcmap1[k].max == INT_MAX)
		q = "infty";
	    else
		sprintf(q = line + 32, "%d", bcmap1[k].max);
	    phgPrintf("    %s:%s -> %s (0x%x)\n", p, q,
				phgBTypesString(bcmap1[k].btype),
				bcmap1[k].btype);
	}
    }
    phgFree(ordering);
...
static const char *
default_bdry_type_handler(OPTION *o, const char *arg, int prefix)
{
    int i;
    ...
    if (arg == NULL) {
	/* print help message */
	char msg[1024], *indent = "    ", *p;
	int len;
	strcpy(msg, indent);
	for (i = (1 << BDRY_SHIFT); i <= BDRY_MASK; i <<= 1) {
	    p = BTypeName(i);
	    if (!strncasecmp(p, "bdry_", 5))
		p += 5;
	    len = strlen(msg);
	    if (len + strlen(p) + 5 + strlen(indent) >= 78) {
		char *q = msg + len - 1;
		while (isspace(*q))
		    q--;
		q[1] = '\0';
		phgOptionsPrintfHelp(o, msg);
		strcpy(msg, indent);
	    }
	    sprintf(msg + len, "\"%s\", ", p);
	    for (p = msg + len; *p != '\0'; p++)
	...
	while (isspace(*p))
	    *(p--) = '\0';
	phgOptionsPrintfHelp(o, msg);
	phgOptionsPrintfHelp(o, "separated by the operator '+' or '|'");
	return NULL;
    }

    if (arg[0] != '\0') {
	if ((i = phgBTypesValue(arg)) < 0)
	    return NULL;
	default_bdry_type = i;
    }
...
phgImportSetDefaultBdryType(BTYPE type)
{
    ...
    assert(type >= (1 << BDRY_SHIFT) && type <= BDRY_MASK);

    oldtype = default_bdry_type;
    default_bdry_type = type;    
...
{
    if (((const BCMAP *)p0)->min > ((const BCMAP *)p1)->max)
	return 1;
    if (((const BCMAP *)p0)->max < ((const BCMAP *)p1)->min)
	return -1;
    return 0;
}
...
BOOLEAN
phgImport_(GRID *g, const char *filename, MPI_Comm comm_world)
{
    FILE *fp;
    char line[128], magic[128];
    BOOLEAN ret = FALSE;
    INT i;
    int flag, nprocs, rank;
    static BOOLEAN initialized = FALSE;

    if (!initialized) {
	initialized = TRUE;
	assert(sizeof(BTYPE) < sizeof(int) &&
		(1 << (sizeof(BTYPE) * 8 - 1)) >= UNDEFINED);
	phgOptionsRegisterHandler("-default_bdry_type",
				  "Default boundary type, the argument "
				  "consists of the following keywords:",
				  default_bdry_type_handler, FALSE);
	phgOptionsRegisterFilename("-bcmap_file", "Load bcmap from the "
				   "specified file, this option is only "
	...
	phgOptionsRegisterNoArg("-auto_bcmap", "When loading an Medit mesh, "
				...
				"exists, this option is only effective "
				"for Medit and Gambit formats", &auto_bcmap);
	phgOptionsRegisterNoArg("-allow_degenerate_neighbours",
				"Whether allow degenerate neighbours, "
				"i.e., two elements connected only by a "
				"common vertex or edge (see "
				"test/vertex-neighbours.mesh and "
				"test/edge-neighbours.mesh). "
				"The present implementation is a temporary "
				"workaround which does not ensure correctness "
				"of the result.", &allow_degenerate_neighbours);
	return TRUE;
    }
    ...
    if (phgVerbosity > 0)
	phgPrintf("Default boundary type: %s (%d).\n",
			phgOptionsGetHandler("-default_bdry_type"),
			default_bdry_type);
...
hamilton:
	...
	MPI_Bcast(&ret, sizeof(ret), MPI_BYTE, 0, comm_world);
	if (nprocs > 1) /* force grid distribution */
	    phgRedistributeGrid_(g, comm_world, NULL);
    }
#else	/* USE_MPI */
    Unused(nprocs);
#endif	/* USE_MPI */
